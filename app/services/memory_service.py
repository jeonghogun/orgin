...
from app.models.memory_schemas import (
    ConversationContext, UserProfile, MemoryEntry,
    ContextQuery, ContextUpdate
)
...
class MemoryService:
    ...
    def __init__(self, db_path: str = "data/memory.db", redis_url: str = "redis://localhost:6379"):
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # 인덱스 생성
                conn.execute("CREATE INDEX IF NOT EXISTS idx_context_room_user ON conversation_contexts(room_id, user_id)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_memory_room_user ON memory_entries(room_id, user_id)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_memory_expires ON memory_entries(expires_at)")
    ...
    async def update_context(self, context_update: ContextUpdate) -> bool:
        ...
        try:
            ...
            # 기존 맥락 조회 또는 새로 생성
            existing = await self.get_context(context_update.room_id, context_update.user_id)
            ...
            async with self.get_redis_client() as redis_client:
                if redis_client:
                    cache_key = f"context:{context_update.room_id}:{context_update.user_id}"
    ...
    async def set_memory(self, room_id: str, user_id: str, key: str, value: str, 
                        importance: float = 1.0, ttl: Optional[int] = None) -> bool:
        """메모리 설정"""
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (memory_id, room_id, user_id, key, value, importance, expires_at, current_time))
                conn.commit()
    ...
    async def get_relevant_memories(self, room_id: str, user_id: str, 
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor = conn.execute("""
                    ...
                    LIMIT ?
                """, (
                    room_id, user_id, get_current_timestamp(),

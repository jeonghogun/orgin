...
class RAGService:
        ...
        try:
            ...
            if self._needs_search(rag_context):
                ...
                if query:
                    ...
                    filtered_results = self._filter_and_rank_search_results(raw_results, query)
            ...
            if self._needs_wiki(rag_context):
                ...
                if topic:
                    ...
                    if wiki_summary:
                        rag_context.wiki_summary = self._validate_wiki_content(wiki_summary)
    ...
    def _extract_search_query(self, rag_context: RAGContext) -> Optional[str]:
        ...
        # 불필요한 단어 제거
        stop_words = ["이", "가", "을", "를", "의", "에", "로", "으로", "와", "과", "도", "만", "은", "는"]
    ...
    def _build_rag_prompt(self, rag_context: RAGContext) -> str:
        ...
        # 대화 맥락 정보
        if rag_context.conversation_context and rag_context.conversation_context.summary:
            prompt_parts.append(f"이전 대화 맥락: {rag_context.conversation_context.summary}")
        ...
        if rag_context.search_results:
            # 검색 결과를 요약하여 제공
            summarized_results = self._summarize_search_results(rag_context.search_results)
            prompt_parts.append(f"🔍 검색 결과 요약:\n{summarized_results}")
        else:
            prompt_parts.append("⚠️ 검색 결과가 없습니다. 일반적인 지식으로 답변하겠습니다.")
    ...
    def _extract_key_topics(self, text: str) -> List[str]:
        """주요 주제 추출"""
        keywords = [
            "시간", "날씨", "검색", "위키", "이름", "프로젝트", "일정",
            "음식", "영화", "음악", "운동", "건강", "여행", "취미",
            "일", "학습", "기술", "AI", "인공지능", "프로그래밍",
            "최신", "트렌드", "뉴스", "정보", "방법", "기법"
        ]
    ...
    def _filter_and_rank_search_results(self, results: List[Dict[str, str]], query: str) -> List[Dict[str, str]]:
        """검색 결과 필터링 및 랭킹"""
        if not results:
            return []
        ...
        for result in results:
            ...
            title_match = sum(1 for word in query_words if any(word in title_word for title_word in title_words))
            score += title_match * 10
            
            # 2. 스니펫 관련성 점수
            snippet_words = snippet.split()
            snippet_match = sum(1 for word in query_words if any(word in snippet_word for snippet_word in snippet_words))
            score += snippet_match * 5
            
            # 3. 도메인 신뢰성 점수
            trusted_domains = [
                'wikipedia.org', 'stackoverflow.com', 'github.com', 'medium.com',
                'techcrunch.com', 'arstechnica.com', 'ieee.org', 'acm.org',
            ...
            # 5. 스팸/광고 필터링
            spam_keywords = ['click', 'buy', 'discount', 'sale', 'advertisement', 'sponsored']

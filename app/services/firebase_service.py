...
class FirebaseService:
    ...
    async def save_panel_report(self, review_id: str, round_number: int, panel_name: str, report_data: Dict[str, Any]) -> str:
        """Save panel report to Firestore"""
        try:
            report_ref = self.db.collection('reviews').document(review_id).collection('reports').document()
    ...
    async def save_consolidated_report(self, review_id: str, round_number: int, report_data: Dict[str, Any]) -> str:
        """Save consolidated report to Firestore"""
        try:
            report_ref = self.db.collection('reviews').document(review_id).collection('consolidated_reports').document()
            report_data['report_id'] = report_ref.id
            report_data['round_number'] = round_number
            report_data['timestamp'] = datetime.utcnow().isoformat() + 'Z'
            report_ref.set(report_data)
    ...
    async def get_consolidated_report(self, review_id: str, round_number: int) -> Optional[Dict[str, Any]]:
        """Get consolidated report from Firestore"""
        try:
            reports_ref = self.db.collection('reviews').document(review_id).collection('consolidated_reports')
            reports = reports_ref.where('round_number', '==', round_number).limit(1).stream()
    ...
    async def save_final_report(self, review_id: str, report_data: Dict[str, Any]) -> str:
        """Save final report to Firestore"""
        try:
            report_ref = self.db.collection('reviews').document(review_id).collection('final_reports').document()
            report_data['report_id'] = report_ref.id
            report_data['timestamp'] = datetime.utcnow().isoformat() + 'Z'
            report_ref.set(report_data)
    ...
    async def get_final_report(self, review_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            reports_ref = self.db.collection('reviews').document(review_id).collection('final_reports')

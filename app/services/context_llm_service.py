...
class ContextLLMService:
    ...
    def _build_context_prompt(
        ...
        # 시스템 역할 설정
        prompt_parts.append("당신은 맥락을 이해하고 기억하는 친근한 AI 어시스턴트입니다.")
    ...
    async def _update_context_after_response(
        ...
        try:
            ...
            context_summary = f"사용자가 '{user_message}'라고 말했고, AI가 응답했습니다."
            ...
            if "내 이름은" in message or "저는" in message:
                ...
                for i, word in enumerate(words):
                    if word in ["이름은", "저는"] and i + 1 < len(words):
                        name = words[i + 1].replace("입니다", "").replace("이야", "").replace("야", "")

"""
Room-related API endpoints - Refactored for dependency injection and async safety.
"""
...
async def create_room(
    ...
    new_room = await asyncio.to_thread(
        ...
        room_id=room_id, name=room_request.name, owner_id=user_id,
        room_type=room_request.type, parent_id=room_request.parent_id,
    )
    return new_room
...
async def update_room_name(
    ...
    if room.owner_id != user_id:
        raise ForbiddenError()
    success = await asyncio.to_thread(storage_service.update_room_name, room_id, room_request.name, owner_id=user_id)
    if not success:
        raise NotFoundError("room", room_id)
    updated_room = await asyncio.to_thread(storage_service.get_room, room_id)
    if not updated_room:
        raise NotFoundError("room", room_id)
    return updated_room
...
async def delete_room(
    ...
    success = await asyncio.to_thread(storage_service.delete_room, room_id, owner_id=user_id)
    if not success:
        raise NotFoundError("room", room_id)
...
async def export_room_data(
    ...
    full_reviews = await asyncio.to_thread(storage_service.get_full_reviews_by_room, room_id)
    exportable_reviews = []
    for review in full_reviews:
        ...
        exportable_reviews.append(
            ExportableReview(
                topic=review.topic, status=review.status, created_at=review.created_at,
                final_summary=summary, next_steps=next_steps
            )
        )
    export_data = ExportData(
        room_id=room_id, room_name=room.name, messages=messages,
        reviews=exportable_reviews, export_timestamp=get_current_timestamp()
    )
    if format == "markdown":
        markdown_content = _format_export_as_markdown(export_data)
        return Response(
            content=markdown_content, media_type="text/markdown",

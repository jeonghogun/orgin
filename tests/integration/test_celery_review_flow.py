...
async def test_create_review_starts_celery_flow(authenticated_client: TestClient):
    ...
    # --- 2. Mock the first Celery task ---
    with patch("app.services.review_service.run_initial_panel_turn.delay") as mock_delay:
        ...
        call_args = mock_delay.call_args[1] # .delay uses keyword args
        assert call_args["review_id"] == review_data["review_id"]
        assert call_args["topic"] == review_topic
        assert call_args["instruction"] == review_instruction

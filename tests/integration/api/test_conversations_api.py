...
def test_create_and_list_threads(authenticated_client: TestClient):
    response = authenticated_client.post(f"/api/convo/subrooms/{MOCK_SUB_ROOM_ID}/threads", json={"title": "Integration Test Thread"})
    assert response.status_code == 201
    created_thread = response.json()
    thread_id = created_thread["id"]

    response = authenticated_client.get(f"/api/convo/subrooms/{MOCK_SUB_ROOM_ID}/threads")
...
def test_full_message_and_stream_flow(authenticated_client: TestClient):
    res = authenticated_client.post(f"/api/convo/subrooms/{MOCK_SUB_ROOM_ID}/threads", json={"title": "SSE Flow Test"})
...
def test_edit_and_regenerate_flow(authenticated_client: TestClient):
    ...
    res = authenticated_client.post(f"/api/convo/subrooms/{MOCK_SUB_ROOM_ID}/threads", json={"title": "Edit Test"})
    thread_id = res.json()["id"]
...
def test_zip_export(authenticated_client: TestClient):
    ...
    """
    res = authenticated_client.post(f"/api/convo/subrooms/{MOCK_SUB_ROOM_ID}/threads", json={"title": "Export Test"})
    thread_id = res.json()["id"]
...
def test_async_export_flow(authenticated_client: TestClient):
    ...
    """
    # 1. Create a thread
    res = authenticated_client.post(f"/api/convo/subrooms/{MOCK_SUB_ROOM_ID}/threads", json={"title": "Async Export Test"})
    thread_id = res.json()["id"]
...
def test_diff_view_flow(authenticated_client: TestClient):
    ...
    res = authenticated_client.post(f"/api/convo/subrooms/{MOCK_SUB_ROOM_ID}/threads", json={"title": "Diff Test"})
    thread_id = res.json()["id"]

...
def test_create_and_list_threads(authenticated_client: TestClient):
    response = authenticated_client.post(f"/api/rooms/{MOCK_SUB_ROOM_ID}/threads", json={"title": "Integration Test Thread"})
    assert response.status_code == 201
    created_thread = response.json()
    thread_id = created_thread["id"]

    response = authenticated_client.get(f"/api/rooms/{MOCK_SUB_ROOM_ID}/threads")
...
def test_full_message_and_stream_flow(authenticated_client: TestClient):
    res = authenticated_client.post(f"/api/rooms/{MOCK_SUB_ROOM_ID}/threads", json={"title": "SSE Flow Test"})
    ...
    with patch("app.api.routes.conversations.get_llm_adapter") as mock_get_adapter:
        ...
        res = authenticated_client.post(f"/api/threads/{thread_id}/messages", json={"content": "Hello"})
        assert res.status_code == 200
        assistant_message_id = res.json()["messageId"]

        with authenticated_client.stream("GET", f"/api/messages/{assistant_message_id}/stream") as response:
            assert response.status_code == 200
    ...
    res = authenticated_client.get(f"/api/threads/{thread_id}/messages")
    messages = res.json()
...
def test_edit_and_regenerate_flow(authenticated_client: TestClient):
    ...
    res = authenticated_client.post(f"/api/rooms/{MOCK_SUB_ROOM_ID}/threads", json={"title": "Edit Test"})
    assert res.status_code == 201
    thread_id = res.json()["id"]
    res = authenticated_client.post(f"/api/threads/{thread_id}/messages", json={"content": "Original message"})
    ...
    res = authenticated_client.patch(f"/api/messages/{original_user_message_id}", json={"content": "Edited message"})
    assert res.status_code == 200
...
def test_zip_export(authenticated_client: TestClient):
    ...
    """
    res = authenticated_client.post(f"/api/rooms/{MOCK_SUB_ROOM_ID}/threads", json={"title": "Export Test"})
    assert res.status_code == 201
    thread_id = res.json()["id"]
    authenticated_client.post(f"/api/threads/{thread_id}/messages", json={"content": "Some content for export"})

    res = authenticated_client.get(f"/api/threads/{thread_id}/export?format=zip")
    assert res.status_code == 200
...
def test_async_export_flow(authenticated_client: TestClient):
    ...
    """
    # 1. Create a thread
    res = authenticated_client.post(f"/api/rooms/{MOCK_SUB_ROOM_ID}/threads", json={"title": "Async Export Test"})
    assert res.status_code == 201
    thread_id = res.json()["id"]

    # 2. Post a job
    res = authenticated_client.post(f"/api/threads/{thread_id}/export/jobs", params={"format": "json"})
    assert res.status_code == 202
    job_id = res.json()["jobId"]
    assert job_id.startswith("exp_")

    # 3. Check job status (since celery is eager, it should be done)
    res = authenticated_client.get(f"/api/export/jobs/{job_id}")
    assert res.status_code == 200
    job = res.json()
...
def test_diff_view_flow(authenticated_client: TestClient):
    ...
    res = authenticated_client.post(f"/api/rooms/{MOCK_SUB_ROOM_ID}/threads", json={"title": "Diff Test"})
    assert res.status_code == 201
    thread_id = res.json()["id"]
    res = authenticated_client.post(f"/api/threads/{thread_id}/messages", json={"content": "original content"})
    ...
    # 3. Edit the message
    res = authenticated_client.patch(f"/api/messages/{orig_user_msg_id}", json={"content": "edited content"})
    ...
    res = authenticated_client.get(f"/api/messages/{edited_user_msg_id}/versions")
    assert res.status_code == 200
    versions = res.json()
    ...
    # 5. Get the diff
    res = authenticated_client.get(f"/api/messages/{versions[1]['id']}/diff?against={versions[0]['id']}")
    assert res.status_code == 200
    diff = res.json()["diff"]

...
# Generate unique user ID for each test session
USER_ID = f"test-user-{int(time.time())}-{str(uuid.uuid4())[:8]}"
...
def test_db():
    """
    Use separate test PostgreSQL database for complete isolation.
    """
    class MockDB:
        def url(self):
            # Use separate test database on different port for complete isolation
            return "postgresql://test_user:test_password@localhost:5434/test_origin_db"
...
def setup_test_environment(test_db):
    ...
    # Override ALL database settings for tests to use direct PostgreSQL connection
    ...
    # Override database connection settings for tests - use separate test database
    ...
    # Force direct database connection (bypass pgbouncer)
    import os
    os.environ["DATABASE_URL"] = test_db.url()
    os.environ["POSTGRES_HOST"] = "localhost"
    os.environ["POSTGRES_PORT"] = "5434"
    os.environ["POSTGRES_USER"] = "test_user"
    os.environ["POSTGRES_PASSWORD"] = "test_password"
    os.environ["POSTGRES_DB"] = "test_origin_db"
